...
export const info = {
  ...
  input: {
    ...
    properties: {
      dirpath: {
        type: 'string',
        description:
          "The path to the directory whose contents will be read. This can be an absolute path (e.g., '/home/user/docs') or a relative path (e.g., './docs') based on the current working directory."
      }
    },
    required: ['dirpath']
  },
  options: {
    includeHidden: {
      type: 'boolean',
      description:
        "Controls whether hidden files and directories (e.g., those starting with a dot, such as '.gitignore') are included in the output. By default, this is set to false, excluding hidden items for a cleaner result."
    },
    recursive: {
      type: 'boolean',
      description:
        'Determines whether the script will recursively traverse subdirectories within the specified directory. When set to true, it includes the contents of all nested directories; otherwise, it only processes the top-level contents. Default is false.'
    }
  },
  output: {
    type: 'object',
    properties: {
      ...
      timestamp: {
        ...
        format: 'date-time',
        description:
          "The date and time when the directory contents were read, formatted as an ISO 8601 string (e.g., '2023-10-15T14:30:00Z')."
      },
      files: {
        type: 'array',
        items: {
            ...
            existing: {
              ...
              properties: {
                ...
                extension: {
                  type: 'string',
                  description:
                    "The file extension, if present (e.g., 'txt'), or an empty string if none."
                },
                directory: {
                  type: 'string',
                  description:
                    "The path to the parent directory of the file (e.g., '/home/user/docs')."
                },
                fullpath: {
                  type: 'string',
                  description:
                    "The complete path to the file (e.g., '/home/user/docs/document.txt')."
                },
                filesize: {
                  type: 'number',
                  description: 'The size of the file in bytes (e.g., 1024 for a 1KB file).'
                },
                modified: {
                  type: 'string',
                  format: 'date-time',
                  description:
                    "The last modification time of the file, in ISO 8601 format (e.g., '2023-10-15T12:00:00Z')."
                }
              }
            },
            error: {
              type: 'string',
              description:
                'An optional error message if the file’s metadata could not be retrieved (e.g., due to permission issues).'
            }
          }
        }
      },
          ...
          properties: {
            existing: {
              ...
              properties: {
                dirname: {
                  type: 'string',
                  description: "The name of the subdirectory (e.g., 'subfolder')."
                },
                directory: {
                  type: 'string',
                  description:
                    "The path to the parent directory of the subdirectory (e.g., '/home/user/docs')."
                },
                fullpath: {
                  type: 'string',
                  description:
                    "The complete path to the subdirectory (e.g., '/home/user/docs/subfolder')."
                },
                isSymlink: {
                  type: 'boolean',
                  description:
                    'Indicates whether the subdirectory is a symbolic link (true) or a regular directory (false).'
                }
              }
            }
          }
